...
import { CourseProgressDashboard } from '../components/courses/CourseProgressDashboard';
...
export const CourseProgressPage: React.FC = () => {
  ...
  if (!isAuthenticated) {
    return <Navigate to="/login" state={{ returnTo: `/courses/${courseId}/progress` }} replace />;
  }
  ...
  return (
    <>
      <SEOHead
        title={t('courses.progressTitle', { course: title })}
        description={t('courses.progressDescription', { course: title })}
        keywords={`${title}, course progress, learning progress, autism education`}
      />
